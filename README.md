...
- **Pure-Zig source, statically-linked Linux binary:** zquic depends on no third-party C libraries — no OpenSSL, no BoringSSL, no quictls, no nghttp3. `src/compat.zig` dispatches every syscall by target:
  - **Linux:** raw `std.os.linux.*` syscalls. The build links no libc; `file zig-out/bin/server` reports `statically linked`. Hostname resolution is pure Zig via `/etc/hosts` plus IP literals (covers loopback, the interop runner's docker names, and any `/etc/hosts` entry); a UDP DNS client is the natural follow-up.
  - **macOS / iOS / tvOS / watchOS / visionOS:** Apple's `libSystem.B.dylib` is the only Apple-supported syscall interface — direct kernel traps are explicitly unstable, code-signing/notarization rejects bypass paths, and Mach-O has no static-link option. Every native macOS binary (including Rust, Go, and every other QUIC implementation) links libSystem. zquic does the same: on Darwin targets the build links libSystem and `compat.zig` dispatches to `std.c.*`. The Zig source remains 100% pure-Zig with no third-party C dependencies.
