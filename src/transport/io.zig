            ...
            if (ft == 0x02 or ft == 0x03) {
                ...
                for (&conn.http09_slots) |*slot| {
                    if (slot.awaiting_fin_ack and slot.fin_pkt_pn <= largest_ack) {
                        ...
                        // per-stream `SendBuffer` slot so the slot pool doesn't
                        // accumulate dead entries (the interop `multiplexing`
                        // test serves >64 files on the same connection and
                        // otherwise wedges with `pending-stream-send queue
                        // full`).
